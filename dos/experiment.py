...
# from dos.market_simple import *
from dos.market import *
...
def run_experiment(run, market):
    ...
    for i in range(N_AGENTS):
        action_means[i] = INIT_MEAN
        action_stds[i] = INIT_STD
        trade_means[i] = INIT_TRADE_MEAN
    ...
    for step in range(STEPS):
        ...
        # TODO: Why copy samples to actions?
        actions = [samples[i] for i in range(N_AGENTS)]

        # clip actions
        actions = np.clip(actions, A_MIN, A_MAX)

        # evaluate actions
        values = market.utilities(actions)

        # clip trades
        for agent in range(N_AGENTS):
            trades[agent] = np.clip(trades[agent], 0, values[agent])
            # trades[agent] = np.clip(trades[agent], 0, 10000)
            # trades[agent] = np.random.uniform(size=SAMPLES)
            # trades[agent] = np.random.uniform(0, values[agent], size=SAMPLES)
            # trades[agent] = values[agent]
            # trades[agent] = (float(N_AGENTS - 1.) / N_AGENTS) * values[agent]

        # trade
        for i in range(N_SHARING):
            values[i] = values[i] - trades[i] + np.mean(
                [trades[j] for j in trades if j != i and j < N_SHARING])
            log = {'run': run, 'type': 'sharing', 'step': step,
                   'mean_trade': np.mean(trades[i]),
                   'n_agents': N_AGENTS,
                   'n_share': N_SHARING}
        ...
        for agent in range(N_AGENTS):
            ...
            if agent < N_SHARING:
                ...
                agent_type = 'defect'
            log = {'run': run, 'type': agent_type, 'step': step,
                   'mean_value': mean_vs[agent], 'n_agents': N_AGENTS,
                   'n_share': N_SHARING}
            value_log.append(log)

            sort = sorted(zip(v, zip(samples[agent], trades[agent])),
                          key=lambda t: t[0])  # sorts ascending by value, low values first
        ...
        log = {'run': run, 'type': 'all', 'step': step,
               'mean_value': value, 'n_agents': N_AGENTS,
               'n_share': N_SHARING}
        value_log.append(log)
...
for run in tqdm(range(RUNS)):
    market = Market(N_AGENTS)
    for fraction_sharing in [0, .2, .4, .6, .8, 1]:
        N_SHARING = int(N_AGENTS * fraction_sharing)
        run_experiment(run, market)
...
df.to_csv(result_dir + '/trades.csv', sep='\t', encoding='utf-8')

from dos.plotting import *
